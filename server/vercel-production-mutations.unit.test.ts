...
describe('production-equivalent Vercel mutation smoke',()=>{
  it('persists every core pilot mutation through the Web Handler and Upstash REST stores',async()=>{
    ...
    const rescheduledStart='2031-11-03T16:00:00.000Z';expect((await jsonRequest(coldTeacher,'v1/teacher/sessions/71000000-0000-4000-8000-000000000001/reschedule','POST',{startsAt:rescheduledStart,endsAt:'2031-11-03T17:30:00.000Z',reason:'Production smoke'},teacherToken)).status).toBe(204);
    const coldSchedule=await request(await createAndTrack(env,sessions,limiter),'v1/schedule',{headers:{authorization:`Bearer ${studentToken}`}});expect((await coldSchedule.json() as {upcoming:Array<{id:string;startsAt:string}>}).upcoming).toContainEqual(expect.objectContaining({id:'71000000-0000-4000-8000-000000000001',startsAt:rescheduledStart}));
    ...
    const finalTeacher=await request(await createAndTrack(env,sessions,limiter),'v1/teacher/bootstrap',{headers:{authorization:`Bearer ${teacherToken}`}});const finalData=await finalTeacher.json() as {sessions:Array<{id:string;attendance:Array<{studentId:string;status:string}>}>;students:Array<{id:string;progressPercent:number;projectTitle:string|null}>};expect(finalData.sessions.find(item=>item.id==='71000000-0000-4000-8000-000000000001')?.attendance).toContainEqual(expect.objectContaining({studentId:DEV_IDS.user,status:'present'}));expect(finalData.students.find(item=>item.id===DEV_IDS.user)).toMatchObject({progressPercent:13,projectTitle:'Updated production project'});
