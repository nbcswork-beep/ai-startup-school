...
export class MemoryRepository implements AppRepository {
  ...
  async getAdminWorkspace(userId:string):Promise<AdminWorkspaceDto>{
    ...
    const guardians=Object.values(state.directory).filter(item=>item.roles.includes('guardian')).map(item=>{const links=state.guardianRelations.filter(link=>link.guardianId===item.id&&link.status==='active');return{id:item.id,name:item.displayName,firstName:item.firstName,lastName:item.lastName,email:item.email,phone:item.phone,version:item.version,linkedStudents:links.map(link=>state.directory[link.studentId]?.displayName??link.studentId),studentIds:links.map(link=>link.studentId),relationshipStatus:links.length?'active':'revoked',telegramLinked:Boolean(item.telegramId),telegramId:item.telegramId,status:item.status};});
