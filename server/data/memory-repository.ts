...
import {
  absenceWindow, catchUpSince, classReminderWindow, entityStamp, evaluateDelivery,
  homeworkAssignedWindow, homeworkDeadlineWindow, homeworkOverdueWindow, reviewedWindow,
  sessionChangeWindow, shouldEnqueue, weeklyDigestWindow, type NotificationWindow
...
type NotificationCandidate={recipientUserId:string;type:string;relatedEntityId:string|null;scheduledFor:string;dueAt:string;expiresAt:string;entityStamp:string|null;idempotencyKey:string;safeMetadata:{text:string;buttonText?:string;buttonUrl?:string;callbackData?:string;studentId?:string;weekKey?:string}};
...
export class MemoryRepository implements AppRepository {
  ...
  // Reports follow the persistent directory, so a student created in Admin is covered from the
  // moment they are active rather than only the four seeded pilot records.
  async generateTeacherReports(userId:string,groupId:string,periodStart:string,periodEnd:string):Promise<void>{this.requireRole(userId,'teacher');if(groupId!==PILOT.groupId)throw new AppError('GROUP_FORBIDDEN',403,'Група недоступна');await this.runtimeStore.mutate(state=>{for(const student of this.activeStudents(state)){if(state.teacherReports.some(report=>report.studentId===student.id&&report.periodStart===periodStart&&report.periodEnd===periodEnd))continue;state.teacherReports.push({id:randomUUID(),studentId:student.id,studentFirstName:student.displayName,groupId,groupName:PILOT.groupName,periodStart,periodEnd,payload:{classesScheduled:state.classSessions.length,classesAttended:state.attendance.filter(item=>item.studentId===student.id&&['present','late'].includes(item.status)).length,homeworkSubmitted:state.submissions.filter(item=>item.studentId===student.id).length,project:state.projects[student.id]?.find(item=>item.status==='active')?.title??null,projectProgress:this.progressPercent(state,student.id),xpEarned:state.xp[student.id]??0},teacherComment:'',status:'draft',approvedAt:null});}});}
  ...
  async prepareNotificationBatch(now:Date,weeklyDay:number,weeklyHour:number,limit:number):Promise<NotificationDeliveryDto[]>{
    const snapshot=await this.runtimeStore.read();this.syncDirectory(snapshot);
    const nowMs=now.getTime(),since=catchUpSince(snapshot.notificationRuntime?.lastRunAt,now);
    const format=(value:string)=>new Intl.DateTimeFormat('uk-UA',{timeZone:'Europe/Kyiv',day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}).format(new Date(value));
    const candidates:NotificationCandidate[]=[];
    const consider=(window:NotificationWindow,build:()=>NotificationCandidate)=>{if(shouldEnqueue(window,since,now))candidates.push(build());};
    ...
    for(const session of snapshot.classSessions){
      const stamp=entityStamp(snapshot,'student_class_24h',session.id);
      if(['scheduled','rescheduled'].includes(session.status))for(const student of students){
        consider(classReminderWindow(session.startsAt,'24h'),()=>this.notificationCandidate(student.id,'student_class_24h',session.id,`class24:${session.id}:${session.startsAt}:${student.id}`,`🚀 Завтра заняття\n\n${session.title}\n${format(session.startsAt)}`,now,classReminderWindow(session.startsAt,'24h'),stamp,{}));
        consider(classReminderWindow(session.startsAt,'1h'),()=>this.notificationCandidate(student.id,'student_class_1h',session.id,`class1:${session.id}:${session.startsAt}:${student.id}`,`⏰ Через годину починаємо\n\n${session.title}\n${format(session.startsAt)}`,now,classReminderWindow(session.startsAt,'1h'),stamp,session.meetingUrl?{buttonText:'🎥 Приєднатися до заняття',buttonUrl:session.meetingUrl}:{}));
      }
      if(session.changedAt&&['rescheduled','cancelled'].includes(session.status))for(const student of students)for(const guardianId of guardiansFor(student.id)){
        const window=sessionChangeWindow(session.changedAt,session.startsAt);
        consider(window,()=>this.notificationCandidate(guardianId,`guardian_session_${session.status}`,session.id,`session:${session.status}:${session.id}:${session.startsAt}:${session.changedAt}:${guardianId}`,`${session.status==='cancelled'?'❌ Заняття скасовано':'📅 Час заняття змінено'}\n\n${student.displayName} · ${session.title}\n${format(session.startsAt)}`,now,window,null,{studentId:student.id,buttonText:'Відкрити кабінет',callbackData:`parent:student:${student.id}`}));
      }
    }

    for(const homework of snapshot.homework.filter(item=>item.status==='published')){
      const stamp=entityStamp(snapshot,'student_homework_deadline',homework.id),publishAt=homework.publishAt,dueAt=homework.dueAt;
      for(const student of students){
        const attempts=snapshot.submissions.filter(item=>item.studentId===student.id&&item.homeworkId===homework.id),submitted=attempts.length>0;
        if(publishAt)consider(homeworkAssignedWindow(publishAt,dueAt),()=>this.notificationCandidate(student.id,'student_homework_assigned',homework.id,`homework:assigned:${homework.id}:${student.id}`,`📝 Нове домашнє завдання\n\n${homework.title}${dueAt?`\nДедлайн: ${format(dueAt)}`:''}`,now,homeworkAssignedWindow(publishAt,dueAt),stamp,{}));
        if(dueAt&&!submitted){
          consider(homeworkDeadlineWindow(dueAt),()=>this.notificationCandidate(student.id,'student_homework_deadline',homework.id,`homework:deadline:${homework.id}:${dueAt}:${student.id}`,`⏳ Нагадування про домашнє завдання\n\n${homework.title}\nДедлайн: ${format(dueAt)}`,now,homeworkDeadlineWindow(dueAt),stamp,{}));
          consider(homeworkOverdueWindow(dueAt),()=>this.notificationCandidate(student.id,'student_homework_overdue',homework.id,`homework:overdue:${homework.id}:${dueAt}:${student.id}`,`⚠️ Домашнє завдання прострочено\n\n${homework.title}`,now,homeworkOverdueWindow(dueAt),stamp,{}));
        }
      }
    }

    for(const submission of snapshot.submissions)if(submission.review){
      const homework=snapshot.homework.find(item=>item.id===submission.homeworkId),window=reviewedWindow(submission.review.reviewedAt);
      consider(window,()=>this.notificationCandidate(submission.studentId,'student_homework_reviewed',submission.id,`homework:reviewed:${submission.id}:${submission.review!.reviewedAt}`,`✅ Викладач перевірив твоє завдання\n\n${homework?.title??'Домашнє завдання'}\nОцінка: ${submission.review!.score}/10\n${submission.review!.feedback.slice(0,500)}`,now,window,null,{}));
    }

    for(const attendance of snapshot.attendance)if(attendance.status==='absent'){
      const student=snapshot.directory[attendance.studentId],session=snapshot.classSessions.find(item=>item.id===attendance.sessionId);
      if(!student||!session)continue;
      const window=absenceWindow(attendance.confirmedAt);
      for(const guardianId of guardiansFor(student.id))consider(window,()=>this.notificationCandidate(guardianId,'guardian_absent_lesson',attendance.id,`attendance:absent:${attendance.id}:${attendance.confirmedAt}:${guardianId}`,`📅 ${student.displayName} пропустив(ла) заняття\n\n${session.title}\nМатеріали для самостійного опрацювання вже доступні.`,now,window,null,{studentId:student.id,buttonText:'Відкрити кабінет',callbackData:`parent:student:${student.id}`}));
    }

    // The digest is anchored to its scheduled instant rather than to the moment the worker happens
    // to run, so a scheduler that is hours late still delivers the same week's summary exactly once.
    for(const occurrence of this.weeklyDigestOccurrences(now,since,weeklyDay,weeklyHour)){
      const window=weeklyDigestWindow(occurrence.dueAt);
      for(const link of snapshot.guardianRelations.filter(item=>item.status==='active')){
        const guardian=snapshot.directory[link.guardianId],student=snapshot.directory[link.studentId];
        if(!guardian||guardian.status!=='active'||!student||student.status!=='active'||!student.roles.includes('student'))continue;
        consider(window,()=>this.notificationCandidate(guardian.id,'guardian_weekly_digest',student.id,`weekly:${guardian.id}:${student.id}:${occurrence.weekKey}`,this.weeklyDigestText(snapshot,student.id,occurrence.weekKey),now,window,null,{studentId:student.id,weekKey:occurrence.weekKey,buttonText:'Відкрити кабінет',callbackData:`parent:student:${student.id}`}));
      }
    }

    await this.runtimeStore.mutate(state=>{
      const keys=new Set(state.notifications.map(item=>item.idempotencyKey));
      for(const candidate of candidates){
        if(keys.has(candidate.idempotencyKey))continue;
        const telegram=this.telegramForUser(state,candidate.recipientUserId);
        if(!telegram)continue;
        state.notifications.push({id:randomUUID(),recipientUserId:candidate.recipientUserId,recipientTelegramId:telegram,type:candidate.type,relatedEntityId:candidate.relatedEntityId,scheduledFor:candidate.scheduledFor,sentAt:null,status:'pending',attempts:0,lastAttemptAt:null,nextAttemptAt:null,idempotencyKey:candidate.idempotencyKey,safeMetadata:candidate.safeMetadata,errorCode:null,dueAt:candidate.dueAt,expiresAt:candidate.expiresAt,entityStamp:candidate.entityStamp,readAt:null});
        keys.add(candidate.idempotencyKey);
      }
    });

    return this.runtimeStore.mutate(state=>{
      const claimed:NotificationDeliveryDto[]=[];
      for(const item of state.notifications){
        if(claimed.length>=limit)break;
        const retryable=item.status==='pending'||item.status==='failed'&&(!item.nextAttemptAt||Date.parse(item.nextAttemptAt)<=nowMs)||item.status==='processing'&&Boolean(item.lastAttemptAt)&&nowMs-Date.parse(item.lastAttemptAt!)>600_000;
        if(!retryable||item.attempts>=5||Date.parse(item.dueAt??item.scheduledFor)>nowMs)continue;
        const decision=evaluateDelivery(state,item,now);
        if(!decision.deliver){item.status='skipped';item.errorCode=decision.reason;item.nextAttemptAt=null;continue;}
        const telegram=this.telegramForUser(state,item.recipientUserId)!;
        item.recipientTelegramId=telegram;item.status='processing';item.attempts+=1;item.lastAttemptAt=now.toISOString();item.errorCode=null;
        claimed.push({id:item.id,recipientUserId:item.recipientUserId,recipientTelegramId:telegram,type:item.type,relatedEntityId:item.relatedEntityId,text:item.safeMetadata.text,buttonText:item.safeMetadata.buttonText??null,buttonUrl:item.safeMetadata.buttonUrl??null,callbackData:item.safeMetadata.callbackData??null,attempts:item.attempts});
      }
      state.notificationRuntime={lastRunAt:now.toISOString(),lastClaimedCount:claimed.length};
      return claimed;
    });
  }

  /**
   * Heartbeat for the external scheduler. A silent worker is the failure mode that hides every
   * other notification problem, so its last run is surfaced next to the other health checks.
   */
  private notificationWorkerHealth(state:PilotRuntimeState):{status:'ok'|'warning'|'required';label:string}{
    const lastRunAt=state.notificationRuntime?.lastRunAt;
    if(!lastRunAt)return{status:'required',label:'Worker сповіщень ще жодного разу не запускався'};
    const ageMinutes=Math.round((Date.now()-Date.parse(lastRunAt))/60_000);
    if(ageMinutes>180)return{status:'required',label:`Worker сповіщень мовчить ${Math.round(ageMinutes/60)} год`};
    if(ageMinutes>75)return{status:'warning',label:`Останній запуск ${ageMinutes} хв тому`};
    return{status:'ok',label:`Останній запуск ${ageMinutes} хв тому`};
  }

  /** Scheduled weekly-digest instants that fell due within this run's catch-up reach. */
  private weeklyDigestOccurrences(now:Date,since:number,weeklyDay:number,weeklyHour:number):Array<{dueAt:number;weekKey:string}>{
    const occurrences:Array<{dueAt:number;weekKey:string}>=[];
    for(let back=0;back<=8;back+=1){
      const probe=new Date(now.getTime()-back*86_400_000),parts=this.kyivParts(probe);
      if(parts.day!==weeklyDay)continue;
      const dueAt=this.kyivMidnight(parts.date)+weeklyHour*3_600_000;
      if(dueAt<=now.getTime()&&dueAt>=since)occurrences.push({dueAt,weekKey:this.weekKey(parts.date)});
    }
    return occurrences;
  }
  ...
  async listStudentNotifications(userId:string):Promise<StudentNotificationsDto>{
    const state=await this.runtimeStore.read();this.syncDirectory(state);this.requireRole(userId,'student');
    return this.studentNotificationsDto(state,userId);
  }

  async markStudentNotificationsRead(userId:string,notificationIds:string[]):Promise<StudentNotificationsDto>{
    const state=await this.runtimeStore.read();this.syncDirectory(state);this.requireRole(userId,'student');
    const requested=new Set(notificationIds),readAt=nowIso();
    return this.runtimeStore.mutate(next=>{
      for(const item of next.notifications){
        if(item.recipientUserId!==userId||!item.type.startsWith('student_')||item.status==='skipped'||item.readAt)continue;
        if(requested.size===0||requested.has(item.id))item.readAt=readAt;
      }
      return this.studentNotificationsDto(next,userId);
    });
  }
  ...
  private notificationCandidate(recipientUserId:string,type:string,relatedEntityId:string|null,idempotencyKey:string,text:string,now:Date,window:NotificationWindow,entityStamp:string|null,extra:Partial<NotificationCandidate['safeMetadata']>={}):NotificationCandidate{
    return{recipientUserId,type,relatedEntityId,scheduledFor:new Date(window.dueAt).toISOString(),dueAt:new Date(window.dueAt).toISOString(),expiresAt:new Date(window.expiresAt).toISOString(),entityStamp,idempotencyKey,safeMetadata:{text,...extra}};
  }
  ...
  private studentNotificationsDto(state:PilotRuntimeState,userId:string):StudentNotificationsDto{
    const titles:Record<string,string>={
      student_class_24h:'Заняття завтра',
      student_class_1h:'Заняття за годину',
      student_homework_assigned:'Нове домашнє завдання',
      student_homework_deadline:'Нагадування про домашнє',
      student_homework_overdue:'Дедлайн минув',
      student_homework_reviewed:'Домашнє перевірено'
    };
    const items=state.notifications
      .filter(item=>item.recipientUserId===userId&&item.type.startsWith('student_')&&item.status!=='skipped')
      .sort((a,b)=>Date.parse(b.dueAt??b.scheduledFor)-Date.parse(a.dueAt??a.scheduledFor))
      .slice(0,50)
      .map(item=>{
        const lines=item.safeMetadata.text.split(/\r?\n/).map(line=>line.trim()).filter(Boolean);
        const submission=item.type==='student_homework_reviewed'?state.submissions.find(value=>value.id===item.relatedEntityId):undefined;
        const homeworkId=item.type==='student_homework_reviewed'?submission?.homeworkId??null:item.type.startsWith('student_homework_')?item.relatedEntityId:null;
        const destination:StudentNotificationDto['destination']=homeworkId?{tab:'learn',homeworkId}:item.type.startsWith('student_class_')?{tab:'learn',homeworkId:null}:item.type.startsWith('student_project_')?{tab:'project',homeworkId:null}:null;
        return{id:item.id,type:item.type,title:titles[item.type]??'Сповіщення',description:lines.slice(1).join(' · ').slice(0,320)||lines[0]?.replace(/^[^\p{L}\p{N}]+/u,'').slice(0,320)||'',occurredAt:item.dueAt??item.scheduledFor,readAt:item.readAt,destination};
      });
