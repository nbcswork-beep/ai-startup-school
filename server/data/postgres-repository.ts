...
export class PostgresRepository implements AppRepository {
  ...
  async getTeacherWorkspace(userId:string):Promise<TeacherWorkspaceDto>{
    return this.withUser(userId,async db=>{
      const teacher=(await db.query(`select u.id,tp.display_name,tp.title,tp.timezone from public.users u join public.teacher_profiles tp on tp.user_id=u.id where u.id=$1 and u.kind in ('teacher','admin') and tp.is_active`,[userId])).rows[0];
