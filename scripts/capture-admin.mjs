...
try{
  for(const [width,height,currentRoutes] of [[1440,900,routes],[1280,800,routes],[1024,768,tabletRoutes]]){
    ...
    if(width===1440){await command('Runtime.evaluate',{expression:`location.hash='#students'`});await sleep(250);await command('Runtime.evaluate',{expression:`document.querySelector('[data-action="account"]')?.click()`});await sleep(150);const shot=await command('Page.captureScreenshot',{format:'png',fromSurface:true,captureBeyondViewport:false});writeFileSync(join(output,'admin-1440-confirmation.png'),Buffer.from(shot.data,'base64'));report.push({width,route:'confirmation',modal:Boolean((await command('Runtime.evaluate',{returnByValue:true,expression:`document.querySelector('.modal-layer.open')!==null`})).result.value)})}
  }
